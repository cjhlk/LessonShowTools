    ...
    AppLogService appLogService)
{
    ...
    public async Task ExportDiagnosticData(string path)
    {
        try
        {
            var temp = Directory.CreateTempSubdirectory("LessonShowToolsDiagnosticExport").FullName;
    ...
    public static void ProcessCriticalException(Exception? ex)
    {
        ...
        CopyException();
        var message = $"""
                       很抱歉，LessonShowTools 遇到了无法解决的问题，即将退出。堆栈跟踪信息已复制到剪贴板。点击【确定】将退出应用，点击【取消】将启动调试器。
        ...
        var r = System.Windows.MessageBox.Show(message, "LessonShowTools", MessageBoxButton.OKCancel, MessageBoxImage.Error);
