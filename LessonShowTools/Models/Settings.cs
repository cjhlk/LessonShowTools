...
using LessonShowTools.Core.Helpers.Native;
using LessonShowTools.Core.Models;
...
using LessonShowTools.Shared.Abstraction.Models;
using LessonShowTools.Shared.Enums;
using LessonShowTools.Shared.Models.Notification;
...
public class Settings : ObservableRecipient, ILessonControlSettings, INotificationSettings
{
    ...
    private string _selectedChannel = "127.0.0.1";
    ...
    private string _mainWindowFont = App.IsAssetsTrimmedInternal ? "Microsoft YaHei UI" : "/LessonShowTools;component/Assets/Fonts/#HarmonyOS Sans SC";
    ...
    {
        { "Test", "https://cjhdevact.github.io" }
    };
    ...
    //{
    //   // => CheckReg("LessonShowTools", @"Software\Microsoft\Windows\CurrentVersion\Run");
    //    get => IsTaskExistsInRegistry();
    ...
    //        catch (Exception ex)
    //        {
    //            App.GetService<ILogger<Settings>>().LogError(ex, "无法创建开机自启动。");
    //        }
    //    }
    //}
    ...
    //{
    //    string taskName = "LST";
    //    string taskPath = "\\CJH";  // 子目录路径
    //    string fullTaskName = taskPath + "\\" + taskName;  // 完整任务名称: \CJH\LST
    ...
    //        {
    //            App.GetService<ILogger<Settings>>().LogDebug("错误: 无法从内嵌资源读取 XML 模板文件");
    //            return;
    //        }

    //        App.GetService<ILogger<Settings>>().LogDebug("成功从内嵌资源读取 XML 模板");
    ...
    //            App.GetService<ILogger<Settings>>().LogDebug($"已将 {oldExeName} 替换为: {currentExePath}");
    //        }
    //        else
    //        {
    //            App.GetService<ILogger<Settings>>().LogDebug($"警告: XML 中未找到 {oldExeName}，跳过替换");
    //        }

    //        // 3. 写入临时目录（使用 UTF-16 编码，任务计划要求）
    //        File.WriteAllText(tempXmlPath, xmlContent, Encoding.Unicode);
    //        App.GetService<ILogger<Settings>>().LogDebug($"已创建临时 XML: {tempXmlPath}");
    //        // 4. 删除现有任务（如果存在）
    //        ExecuteSchtasks($"/Delete /TN \"{fullTaskName}\" /F");
    ...
    //    catch (Exception ex)
    //    {
    //        App.GetService<ILogger<Settings>>().LogDebug($"错误: {ex.Message}");
    //        App.GetService<ILogger<Settings>>().LogDebug($"详细信息: {ex.StackTrace}");
    //    }
    //    finally
    //    {
    //        // 6. 清理临时文件
    ...
    //                App.GetService<ILogger<Settings>>().LogDebug($"已清理临时文件: {tempXmlPath}");
    //            }
    //            catch (Exception ex)
    //            {
    //                App.GetService<ILogger<Settings>>().LogDebug($"清理临时文件失败: {ex.Message}");
    //            }
    //        }
    //    }
    //}
    ...
    //{
    //    get => Environment.GetEnvironmentVariable("LessonShowTools_IsSentryEnabled") is "1" or null;
    //    set
    //    {
    //        try
    //        {
    //            var envVar = value ? "1" : "0";
    //            Environment.SetEnvironmentVariable("LessonShowTools_IsSentryEnabled", envVar);
    //            // 因为 Environment.SetEnvironmentVariable 有时会执行很长时间，所以这里要直接修改注册表。
    //            using var reg = Registry.CurrentUser.OpenSubKey(
    //                @"Environment", true);
    //            reg?.SetValue("LessonShowTools_IsSentryEnabled", envVar, RegistryValueKind.String);
    //            OnPropertyChanged();
    //        }
    //        catch (Exception ex)
    //        {
    ...
    {
        get => Environment.GetEnvironmentVariable("LessonShowTools_ShowSellingAnnouncement") is "1" or null;
        set
        {
            try
            {
                var envVar = value ? "1" : "0";
                Environment.SetEnvironmentVariable("LessonShowTools_ShowSellingAnnouncement", envVar);
                // 因为 Environment.SetEnvironmentVariable 有时会执行很长时间，所以这里要直接修改注册表。
                using var reg = Registry.CurrentUser.OpenSubKey(
                    @"Environment", true);
                reg?.SetValue("LessonShowTools_ShowSellingAnnouncement", envVar, RegistryValueKind.String);
                OnPropertyChanged();
            }
            catch (Exception ex)
            {
